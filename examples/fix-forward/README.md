# Worked Example: Fix-Forward Path
...
This example is the secondary worked example.
Read [../happy-path/README.md](../happy-path/README.md) first if you want the
default clean completion flow.

This example shows one process-only artifact set for a change that:
...
- shows a partial checkpoint that needs fix-forward follow-up
...
This example shows the checkpointed execution path plus a fix-forward review
outcome.
